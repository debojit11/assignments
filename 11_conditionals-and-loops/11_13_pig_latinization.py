...
# Manually copy the story text and save it in a variable
story_text = """You would never guess what can happen when you jump into a seemingly shallow puddle at night time!
...
for char in story_text:
    if char.isalpha():  
        ...
        if word:  # If we have a complete word
            ...
            pig_latin_text += pig_latin_word + char
            word = ""  # Reset the word variable
        else:
            pig_latin_text += char  # Append non-alphabetic characters directly

# If there is any word left at the end
if word:
    pig_latin_word = word[1:] + word[0] + "ay"
    pig_latin_text += pig_latin_word

# Print the translated Pig Latin text
